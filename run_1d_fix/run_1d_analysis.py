...
save_get_dir = '/Users/biocomplexity/Projects/SocioCognitiveModeling/NSF-NetCogSys/Products/CAN_Model/CAN_GitData/Hopfield/run_1d_fix-renames/'



'''FROM run_1b_analysis_alt.py'''
'''NETWORK PROPERTIES OF DALEGE GRAPH'''
#GEN DATA
...
'''FROM run_1d_analysis_all.py'''
'''CAPTURE THE K STATES FOR ALL NODES SHARE THIS INFO'''
#read in each analysis file
df_all = pd.read_csv(f'{save_get_dir}df_catch_endstate_analysis_out.csv')
...
#for j in range(0,1): #FOR TESTING ONLY
    ...
    '''FROM run_1d_analysis_all.py'''
    df_target = pd.read_csv(f'{save_get_dir}df_catch_endstate_analysis_out_{target_node_no}.csv')
    df_target_use = df_target[['Endstate','Frequency','Energy']]
    uniq_endstates_decimal = np.array(df_target_use.Endstate)
    uniq_endstates_decimal_freq = np.array(df_target_use.Frequency)#is int64 as should be
...
plt.text(df_sorted.loc[15]['mean_centrality'], df_sorted.loc[15][0], '+',color='blue')
plt.text(df_sorted.loc[17]['mean_centrality'], df_sorted.loc[17][0], '+',color='blue')
plt.text(df_sorted.loc[19]['mean_centrality'], df_sorted.loc[19][0], '+',color='blue')
plt.text(df_sorted.loc[21]['mean_centrality'], df_sorted.loc[21][0], '+',color='blue')
plt.ylim((-0.5,11))
plt.savefig('Fig_WeightHam_1d.png',dpi=400,bbox_inches='tight')
...
plt.text(df_sorted.loc[15]['mean_centrality'], df_sorted.loc[15][1], '+',color='blue')
plt.text(df_sorted.loc[17]['mean_centrality'], df_sorted.loc[17][1], '+',color='blue')
plt.text(df_sorted.loc[19]['mean_centrality'], df_sorted.loc[19][1], '+',color='blue')
plt.text(df_sorted.loc[21]['mean_centrality'], df_sorted.loc[21][1], '+',color='blue')
plt.ylim((0,1.42))
plt.savefig('Fig_EuclidianDist_1d.png',dpi=400,bbox_inches='tight')
...
for i in range(0,len(catch_arr8)):
    if i==9: 
        ...
        plt.plot(catch_arr8[i],'k-+',linewidth=0.9,color='blue')
    else: 
        plt.plot(catch_arr8[i],'k-.',linewidth=0.5,color='black')
    plt.ylim([-0.05,1.05])
    plt.plot(k_states_freq_norm,linewidth=1.3,color='red')
    ...
    custom_lines = [Line2D([0], [0], color='red', lw=1.3),
                Line2D([0], [0], linestyle='-',marker='*',color='green', lw=1),
                Line2D([0], [0], linestyle='-',marker='+',color='blue', lw=1),
                Line2D([0], [0], linestyle='-.',color='black', lw=1)]
    plt.legend(custom_lines, ['Referent', 'Most Central', 'Negative Cluster','Other Nodes'])
plt.savefig('Fig_DistributionByNode_1d.png',dpi=400,bbox_inches='tight')
#EOF