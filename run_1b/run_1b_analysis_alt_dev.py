...
'''THIS SECTION JUST PRINTS TO SCREEN FOR SANITY CHECK'''
...
hd.plot_hist(catch_catch_endstate_clamp_arr[:,8,:]) #SECOND INDEX GIVES ENDSTATES ACROSS REPLICATES FOR NODE i
...
df = pd.DataFrame([catch_weighted_ham,catch_dist]).T
...
plt.savefig('Fig_WeightHam_Alt.png',dpi=400,bbox_inches='tight')
...
plt.savefig('Fig_EuclidianDist_Alt.png',dpi=400,bbox_inches='tight')

'''SIMPLER METHOD PLOTS'''
#MERGE WITH GRAPH INFO IN DATAFRAME
...
plt.ylabel('Weighted Hamming')
plt.scatter(df_sorted.mean_centrality,df_sorted[0],color = 'black')
plt.savefig('Fig_WeightHam_Alt_2.png',dpi=400,bbox_inches='tight')
...
plt.ylabel('Euclidian Distance')
plt.scatter(df_sorted.mean_centrality,df_sorted[1],color = 'black')
plt.savefig('Fig_EuclidianDist_Alt_2.png',dpi=400,bbox_inches='tight')
...
'''SANITY CHECK ON HAMMING ARRAYS FOR EACH NODE
THE LIST catch_catch_hamming_arr has the array for 
each node, in order of j in loop above'''

#FOR NODE 9
catch_catch_hammings_arr[9]

#THE DISTRIBUTIONS BETWEEN K STATES AND A NODE catch_arr4[X] whree X is the node
pd.DataFrame([catch_arr8[11]/catch_arr8[11].sum(),k_states_freq_norm]).round(4).T

#DISTRIBUTION FOR EACH NODE IN COMPARISON
...
for i in range(0,len(catch_arr8)):
    ...
    plt.plot(k_states_freq_norm,'k+',linewidth=1.1,color='red')
