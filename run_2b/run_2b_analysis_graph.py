...
plt.scatter(np.array(range(0,len(wts_min))),wts_mean,color='black',marker='s',alpha=0.4)
plt.scatter(np.array(range(0,len(wts_min))),wts_min,color='black',marker='o',alpha=0.4)
plt.savefig('Mean_hi_Min_hi_draft.png',dpi=250)

'''FOR MANUSCRIPT SUBMISSION MAYBE'''
plt.hist(wts_min[0:60],color='gray',bins=10)
plt.hist(wts_min[60:100],color='black',bins=10)
